...
const DISTANCE_BASE_OBSTACLE =10;
const DISTANCE_BASE_NOT_OBSTACLE =50;
...
void setup() {
    ...
    servo_angle servo_state = servo_angle.FRONTAL;
...
void driveLogic(){
    ...
    if(ultra_distance <= DISTANCE_BASE_OBSTACLE) {
        ...
        turnServo(servo_angle.RIGHT);
        if(ultra_distance >= DISTANCE_BASE_NOT_OBSTACLE){
            turnServo(servo_angle.FRONTAL);
            goRight();
            goForward();  
        }
        
        turnServo(servo_angle.LEFT);
        if(ultra_distance >= DISTANCE_BASE_NOT_OBSTACLE){
            turnServo(servo_angle.FRONTAL);
            goLeft();
            goForward();
